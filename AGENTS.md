...
└── src/
    ...
    │                    #   + cities (top habitability sites, ≥32 km separation, coastal-biased)
    │                    #   + towns (3 per city or 4 standalone if 0 cities, within 30 km of parent,
    │                    #         ≥25 km separation, coastal-biased)
...
| Place 3 towns per city           | `05_terrain.js` → `findTowns()` — 3 towns ≤30 km of each city, or 4 standalone if 0 cities; ≥25 km separation; coastal bonus via `nearWater`                                             |
...
| Prevent forest obscuring cities/towns | `16_mesh_features.js` → `CLEAR_RADIUS_SQ` (5 km) around `region.cities` + `region.towns`                                                               |
...
- **Cities**: `findCities()` picks the top habitability land cells from a shuffled 20%-top candidate pool, greedy-selecting sites with ≥32 km separation. Coastal cells get a +20 score bonus.
- **Towns**: `findTowns()` places either 4 standalone towns (no cities) or 3 towns per city within a 30 km radius. All towns enforce ≥25 km mutual separation. Coastal bonus (+20 habitability) applied.
