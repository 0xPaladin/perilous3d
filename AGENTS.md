...
| Increase mesh detail             | `05_terrain.js` → `npts` (point count, scales with area)                                                                                                                                 |
| Change map size                  | `src/17_gui.js` `Parameters.Map Size (km)` (50–400) → passed as `extentSize` to `buildRegion()` — scales point count, mountain count/dimensions, city/resource/feature counts by area    |
