...
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tune mountain count/distribution | `src/gui/gui.js` `Parameters.Terrain` (wetland/lowland/woodland/highland/wasteland) → mapped in `terrain/terrain.js` `TERRAIN_CONFIGS` (count range, heightScale, rainfall, range params)          |
| Tune mountain size               | `terrain/terrain.js` → `mountains()` → `r: runif(1.2, 2.0 + sizeFactor * 4, rng)` × `TERRAIN_CONFIGS[terrain].heightScale`                                                                   |
| Change mountain shape            | `terrain/terrain.js` → `mountains()` — `peak` (cone) + `skirt` (gaussian) formulas                                                                                                             |
| Tune mountain range clustering   | `terrain/terrain.js` → `mountains()` + `TERRAIN_CONFIGS` (range count, length, width, spread)                                                                                                  |
...
| Generate resources               | `terrain/terrain.js` → `generateResources()` picks Max(#cities,2) unique resource types per region, weighted by biome (e.g., timber in forests, metals in highlands/cold)                                                               |
