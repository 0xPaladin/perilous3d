...
└── src/
    ...
    │                    #   + rivers (flux accumulation) + moisture (Azgaar neighbor-averaging)
...
- **07_mesher.js** scales heights by `HEIGHT_SCALE / maxLandH` for world-Y
...
| Toggle biome cell view           | `index.html` "Biome View" button + `07_mesher.js` → `buildBiomeViewMesh()` (non-indexed per-triangle colors + wireframe) |
